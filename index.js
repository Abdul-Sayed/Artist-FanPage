...
function ready() {
    ...
    //     let ticketItem = button.parentElement.parentElement;
    //     let title = ticketItem.getElementsByClassName("shop-item-title")[0].textContent;
    //     let image = ticketItem.getElementsByClassName("shop-item-img")[0].src;
    //     let price = ticketItem.getElementsByClassName("shop-item-price")[0].textContent;
    ...
    let ticketButtons = document.getElementsByClassName("tour-btn");

    for (let i = 0; i < ticketButtons.length; i++) {
        var button = ticketButtons[i];
        button.addEventListener('click', addTicketToStorage)
    }

    function addTicketToStorage(event) {
        // Get the button element that triggered the event
        let button = event.target;
        // Reference the tour-row container the button belongs to and its date, city, and arena
        let ticketItem = button.parentElement;
        let date = ticketItem.getElementsByClassName("tour-date")[0].textContent;
        let city = ticketItem.getElementsByClassName("tour-city")[0].textContent;
        let arena = ticketItem.getElementsByClassName("tour-arena")[0].textContent;
        // Set the data to local storage to be retreived in store.js
